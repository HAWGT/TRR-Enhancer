...
void* MainModule = GetModuleHandle(nullptr);

BYTE* PatternScan(const char* signature, void* ModuleOverride)
{
    if (!ModuleOverride)
    {
        ModuleOverride = MainModule;
    }
    ...
    auto dosHeader = (PIMAGE_DOS_HEADER)ModuleOverride;
    auto ntHeaders = (PIMAGE_NT_HEADERS)((std::uint8_t*)ModuleOverride + dosHeader->e_lfanew);

    auto sizeOfImage = ntHeaders->OptionalHeader.SizeOfImage;
    auto patternBytes = pattern_to_byte(signature);
    auto scanBytes = reinterpret_cast<std::uint8_t*>(ModuleOverride);
